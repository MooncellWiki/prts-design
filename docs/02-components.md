...
| 页脚 | `.ak-footer` `__inner __brand __col __bottom __bottom-text __icons` + `#footer-places` `#footer-icons` | 与页眉同为「框」：两套主题下都黑，读 `--ak-chrome-bg-solid / --ak-chrome-fg`（不再读 `--ak-bg-inverse`——那是反白组件用的，暗色下是白，跟随系统时页脚曾因此变白）；顶部活动主色斜纹 + 水印；底栏左文字、右 `$wgFooterIcons` 徽章（结构同 Vector/Citizen：`ul#footer-icons > li#footer-*ico > a.cdx-button > img`）——徽章为浅底设计（1.43 的 MediaWiki 徽章是透明底黑字），故仿 Citizen 给链接固定浅色底板，默认灰度 + 62% 透明、悬停/聚焦恢复彩色；右侧为 `.ak-fab` 让位 | ✅ |
